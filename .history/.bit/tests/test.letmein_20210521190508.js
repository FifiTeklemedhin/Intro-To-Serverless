...
access_denied_test = "letmein";
access_granted_test = "letmein";
test1 = "Access denied.";
test2 = "Access granted.";
...
try {
    (async () => {
        ...
        let test = JSON.stringify(data).replace("\"", "");
        if (test.replace("\"", "") == test1) {     
            console.log("Yay! 🎉 We got: " + JSON.stringify(data) + "Your Access Denied test works")
        } else {
...
try {
    (async () => {
        ...
        var data = await resp.text()
        let test = JSON.stringify(data).replace("\"", "");
        if (test.replace("\"", "") == test2) {     
            console.log("Yay! 🎉 We got: " + JSON.stringify(data) + "Your Access Granted test works")
        } else {
